...
// should fall back on gamma 2.2 instead of sRGB for this gamma correction type, but we haven't bothered implementing that (it's not worth it).
	...
	{
    float p = PeakWhiteNits / sRGB_WhiteLevelNits;
		color = min(color, p);
	}
#elif SWAPCHAIN_CLAMP_PEAK == 2
  {
    ...
    float p = PeakWhiteNits / sRGB_WhiteLevelNits;
		if (m > p) color *= p /* * 1.05 */ / m;
	}
#endif

	//clamp BT2020
#if SWAPCHAIN_CLAMP_COLORSPACE > 0
  ...
  // clamp peak
  {
	...
	//black
  color.rgb = 0;
	color.rgb = DrawRect(uv, float4(0.35, 0.47, 0.65, 0.53), color.rgb, 10000.f/sRGB_WhiteLevelNits);
	color.rgb = DrawRect(uv, float4(0.365, 0.483, 0.448, 0.517), color.rgb, PeakWhiteNits*2/sRGB_WhiteLevelNits);
	color.rgb = DrawRect(uv, float4(0.458, 0.483, 0.542, 0.517), color.rgb, PeakWhiteNits/sRGB_WhiteLevelNits);
	color.rgb = DrawRect(uv, float4(0.552, 0.483, 0.635, 0.517), color.rgb, PeakWhiteNits/2/sRGB_WhiteLevelNits);
