...
{
  float4 PS_REG_SSAO_SCREEN_ : packoffset(c0);
  float4 PS_REG_SSAO_PARAMS_ : packoffset(c1);
  float4 PS_REG_SSAO_MV_1_ : packoffset(c2);
  float4 PS_REG_SSAO_MV_2_ : packoffset(c3);
  float4 PS_REG_SSAO_MV_3_ : packoffset(c4);
  float4 SSAO_FRUSTUM_SCALE_ : packoffset(c5);
  float4 SSAO_TEX_COORD_SCALE_ : packoffset(c6);
...
void main_pass_cs(uint2 dtid : SV_DispatchThreadID)
{
    ...
    float3 viewNormal = float3(
        dot(normal, PS_REG_SSAO_MV_1_.xyz), // 1: left, 0: right
        -dot(normal, PS_REG_SSAO_MV_2_.xyz), // 1: bottom, 0: top
        dot(normal, PS_REG_SSAO_MV_3_.xyz)  // 1: grazing, 0: facing/perpendicular
    );
