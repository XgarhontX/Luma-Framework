// ---- Created with 3Dmigoto v1.3.16 on Wed Aug 05 15:06:44 2026
...
//https://github.com/halohlsl/Halo3-Shader-Source/blob/master/hlsl/final_composite_base.hlsl
void main(
  ...
  out float4 o0 : SV_Target0)
{
  ...
  r0.xy = ps_global_is_texture_in_viewport_flags & int2(1,4);
  ...
  // r1.xyz = LocalTexture_color_grading0.Sample(LocalSampler_color_grading0_s, r0.xyz).xyz; //black? then cg_blend_factor.x == 1
  // r0.xyz = LocalTexture_color_grading1.Sample(LocalSampler_color_grading1_s, r0.xyz).xyz;
  // r0.xyz = lerp(r1.xyz, r0.xyz, cg_blend_factor.x);
  LUT(lut0, lut1, cg_blend_factor.x);

  r0.xyz = tmi.x;
  r0.w = dot(r0.xyz, float3(0.298999995,0.587000012,0.114)); // luma for aa
  o0.w = sqrt(r0.w); // encode gamma
